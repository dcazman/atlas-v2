...
app.listen(PORT, () => {
  console.log('atlas-mcp v2 running on port ' + PORT);
  // Boot-time embedding backfill sweep (RAG design, obs 1304/1306, shared).
  // Non-blocking, best-effort: covers observations written before this
  // deploy plus anything that failed to embed inline. Delayed slightly so
  // the model pipeline isn't racing the very first request.
  setTimeout(() => {
    dbMod.backfillMissingEmbeddings().catch((err) => {
      console.error('startup embedding backfill failed:', err.message);
    });
  }, 5000);
});
...
function renderBoard() {
  ...
  const proposalRows = epicProposals.map((p) => {
    ...
    return `<tr${p.status !== 'proposed' ? ' class="closed"' : ''}><td class="id">${p.id}</td><td>${epicBadge(p.status)}</td><td>${actionLabel(p)}</td><td class="tk">${tickets}</td><td>${cap}</td><td>${esc(p.rationale)}</td></tr>`;
  }).join('');
...
<style>
  ...
  tr.inprog td{background:rgba(74,222,128,.08)}
  tr.inprog td:first-child{border-left:3px solid #1e6b3a}
...
<div id="workers" class="panel">
  ...
  <table><thead><tr><th>Token</th><th>Meaning</th></tr></thead><tbody>
    <tr><td class="tk">d</td><td>dismiss, e.g. <code>d t 4</code> = dismiss tray item 4</td></tr>
    ...
    <tr><td class="tk">dis</td><td>alone on its own line &mdash; discuss only, take no action (reads are fine, no board/tray/Jira writes)</td></tr>
